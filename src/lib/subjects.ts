...
const STATUS_META: Record<SubjectStatus, SubjectStatusMeta> = {
  blog: { label: "Có blog", tone: "teal" },
  simulation: { label: "Có simulation", tone: "cyan" },
  "blog-and-simulation": { label: "Blog + simulation", tone: "teal" },
  building: { label: "Đang viết", tone: "amber" },
  none: { label: "Chưa có", tone: "zinc" },
};
...
function inferStatus(
  ...
  if (hasBlog && hasSimulation) return "blog-and-simulation";
  if (hasSimulation) return "simulation";
  if (hasBlog) return "blog";

  if (resourceLinks.length > 0) return "building";
...
export function getCanonicalArticlePath(articleSlug: string): string | null {
  ...
  if (kind === "sim") {
    return `/curriculum/${subjectSlug}/sim/${articleSlug}`;
  }

  return `/curriculum/${subjectSlug}/${articleSlug}`;
}
  ...
  const list = SUBJECT_DEFINITIONS.map((subject) => {
    ...
    const resources = links
      .map((link) => {
        ...
        const routePath =
          link.kind === "sim"
            ? `/curriculum/${subject.slug}/sim/${article.slug}`
            : `/curriculum/${subject.slug}/${article.slug}`;
